...
# ---- Top editorial line ----
draw.rectangle([(0, 0), (W, px(5))], fill=INK)

# ---- Load fonts ----
font_marko    = load_woff2(FONTS / "Paperlogy-7Bold.woff2",     px(120))
font_tagline  = load_woff2(FONTS / "Paperlogy-5Medium.woff2",   px(32))
font_subtitle = load_woff2(FONTS / "Paperlogy-4Regular.woff2",  px(20))
...
for i, color in enumerate([AMBER, BLUE, GREEN]):
    ...
    draw.rounded_rectangle([x, y_logo, x + SQ, y_logo + SQ], radius=px(9), fill=color)
...
# ---- Divider line ----
y_div = px(258)
draw.line([(px(560), y_div), (px(840), y_div)], fill=INK + "40", width=px(1))

# ---- Tagline ----
tagline = "한국어 논문 글쓰기용 로컬 마크다운 뷰어"
tl_bbox = draw.textbbox((0, 0), tagline, font=font_tagline)
tl_w = tl_bbox[2] - tl_bbox[0]
draw.text(((W - tl_w) // 2, px(278)), tagline, font=font_tagline, fill=INK)
...
# Slightly muted
subtitle_color = (15, 22, 36, 150)  # 0f1624 with ~60% alpha, but RGB can't alpha
# Use mid-gray instead
draw.text(((W - st_w) // 2, px(335)), subtitle, font=font_subtitle, fill='#4a556b')
...
draw.text(((W - tag_w) // 2, px(395)), tag_line, font=font_tags, fill='#7a8499')
